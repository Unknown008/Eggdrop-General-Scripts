...
proc bs_flood {nick uhost} {
  ...
  if {[expr [unixtime] - $bs_flood_array([expr $bs(flood_num) - 1])] <= $bs(flood_time)} {
    ...
    if {$bs(ignore)} {
      newignore [join [maskhost *!*[string trimleft $uhost ~]]] $bs(version) flood $bs(ignore_time)
    }
    return 1
  } {
    return 0
...
proc bs_save {} {
  ...
  if {![string match */* $userfile]} {
    set name [lindex [split $userfile .] 0]
  } {
    set temp [split $userfile /]
    set temp [lindex $temp [expr [llength $temp]-1]]
    set name [lindex [split $temp .] 0]
...
proc bs_chon {hand idx} {
  foreach item [dcclist] {
    ...
    if {[lindex $item 0] == $idx} {
      bs_add $hand "[lindex $item 2] [unixtime] chon"
      break
    }
  }
}
...
proc bs_chpt {bot hand args} {
  ...
  if {$old != "0"} {
    bs_add $hand "[join [string trim [lindex $old 1] ()]] [unixtime] chpt $bot"
  }
}
...
proc bs_away {bot idx msg} {
  ...
  set old [split [bs_search ? [string tolower $hand]]]
  if {$old != "0"} {
    bs_add $hand "[join [string trim [lindex $old 1] ()]] [unixtime] away $bot [bs_filt [join $msg]]"
  }
}
bind dcc n|- unseen bs_unseen
proc bs_unseen {hand idx args} {
  global bs_list
  set tot 0 ; set chan [string tolower [lindex $args 0]]
  set id [array startsearch bs_list]
  while {[array anymore bs_list $id]} {
    set item [array nextelement bs_list $id]
  ...
  array donesearch bs_list $id
  if {$tot > 0} {foreach item $remlist {unset bs_list($item)}}
...
proc bs_botsearch {from cmd args} {
  ...
  if {[string match *\\\** $search]} {
    ...
    if {$output != "No matches were found." && ![string match "I'm not on *" $output]} {
      putbot $from "bs_botsearch_reply $command \{$target\} {$nick, $botnick says:  [bs_filt $output]}"
    }
  } {
...
proc bs_pubreq {nick uhost hand chan args no} {
  ...
  if {[onchan $data $chan]} {
    puthelp "$target :$nick, $data is right here!"
    return $bs(logqueries)
  }
...
proc bs_output {chan nick data no} {
  ...
  if {$no == 0} {
    ...
    if {$matches != ""} {
      ...
      if {$len > 99} {
        return [concat I found $len matches to your query\; please refine it to see any output.]
      }
  ...
  if {$temp != 0} {
    ...
    if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0]<1030000} { 
      ...
      if {$seen == ""} {return "$nick, I don't remember seeing $data."}
...
#bugfix: added a join on the $n  (c)
    ...
    switch -- [lindex $data 3] {
      ...
      chon { 
        set output [concat $n ($addy) was last seen joining the partyline [bs_when $time] ago.]
      ...
      chof { 
        set output [concat $n ($addy) was last seen leaving the partyline [bs_when $time] ago.]
        set lnick [string tolower $n]
        foreach item [whom *] {
          if {$lnick == [string tolower [lindex $item 0]]} {
      ...
      chjn { 
        set output [concat $n ($addy) was last seen joining the partyline on $chan [bs_when $time] ago.]
        set lnick [string tolower $n]
        foreach item [whom *] {
          if {$lnick == [string tolower [lindex $item 0]]} {
            set output [concat $output  $n is on the partyline right now.]
      ...
      chpt { 
        set output [concat $n ($addy) was last seen leaving the partyline from $chan [bs_when $time] ago.]
        set lnick [string tolower $n]
        foreach item [whom *] {
          if {$lnick == [string tolower [lindex $item 0]]} {
            set output [concat $output  $n is on the partyline in [lindex $item 1] still.]
...
proc bs_dccstats {hand idx args} {
  putdcc $idx "[bs_stats]"
  return 1
}
...
proc bs_stats {} {
  ...
  while {[array anymore bs_list $id]} {
    ...
    if {$tok == ""} {
      putlog "Damaged seen record: $item"
      continue
    }
    if {[lindex $bs_list($item) 2] < $bs_record} {
      set bs_record [lindex $bs_list($item) 2]
      set name $item
    }
    set addy [string tolower [maskhost [lindex $bs_list($item) 1]]]
    if {[lsearch -exact $temp $addy] == -1} {
      incr totalm
      lappend temp $addy
    }
  }
...
proc bs_pubchanstats {nick uhost hand chan args} {
  ...
  puthelp "$target :[bs_chanstats $chan]"
  return $bs(logqueries)
}
...
proc bs_msgchanstats {nick uhost hand args} {
  global bs
  if {[bs_flood $nick $uhost]} {return 0}
  puthelp "notice $nick :[bs_chanstats [lindex $args 0]]"
  return $bs(logqueries)
}
...
proc bs_chanstats {chan} {
  global bs_list 
  set chan [string tolower $chan] 
  if {![validchan $chan]} {return "I'm not on $chan."}
  set id [array startsearch bs_list] 
  set bs_record [unixtime] 
  set totalc 0 
  set totalm 0 
  set temp ""
  while {[array anymore bs_list $id]} {
    set item [array nextelement bs_list $id] 
    set time [lindex $bs_list($item) 2] 
    if {$time == ""} {continue}
    if {$chan == [string tolower [lindex $bs_list($item) 4]]} {
      if {$time < $bs_record} {set bs_record $time} 
      incr totalc
      set addy [string tolower [maskhost [lindex $bs_list($item) 1]]]
      if {[lsearch -exact $temp $addy] == -1} {
        incr totalm 
...
foreach chan [string tolower [channels]] {
  if {![info exists bs_botidle($chan)]} {
    set bs_botidle($chan) [unixtime]
  }
}
...
proc lastspoke {nick uhost hand chan args} {
  ...
  set chan [string tolower $chan]
  if {[bs_flood $nick $uhost] || [lsearch -exact $bs(no_pub) $chan] >= 0 || ($bs(log_only) != "" && [lsearch -exact $bs(log_only) $chan] == -1)} {return 0}
  if {[lsearch -exact $bs(quiet_chan) $chan] >= 0} {
    set target "notice $nick"
  } {
    set target "privmsg $chan"
  ...
  if {[onchan $data $chan]} { 
    if {$ldata == [string tolower $botnick]} {
      puthelp "$target :$nick, must you waste my time?" 
      return 1
    }
...
proc bs_version {hand idx args} {
  global bs
  putidx $idx "###  Bass's Seen script, $bs(version)."
}
...
proc bs_help_dcc {hand idx args} {
  ...
  switch -- $args {
    ...
    seennick {
      putidx $idx "###  seen <nick>          $bs(version)"
    }
    chanstats {
